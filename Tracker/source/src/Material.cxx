...
//----------------------------------------------------------------
//
Material::Material(const char* name, const char* title, 
...
//----------------------------------------------------------------
//
void Material::Print(Option_t* option) const
{
...
//----------------------------------------------------------------
//
Float_t Material::GetELoss(Float_t p, float m) const
{
  // return energy lost (in MeV cm^2/g) at momentum p for mass p
  float bg = p/m;
  if (!IsELossProvided()) {
    return TrackParameter::BetheBlochSolid(bg);
  }
  // real Bethe-Bloch
...
//
Float_t Material::GetELoss2ETP(Float_t p, float m) const
{
  // return ratio between real BB and TrackParameter energy loss 
  float bg = p/m;
  if (!IsELossProvided()) {
    return 1.;
  }
  ...
  float SP = ((k*GetZ())/(GetA()*b2))*(0.5*TMath::Log( (2*me*b2g2*T) / (meanExcitE*meanExcitE) ) - b2 - densEff);
  return SP/TrackParameter::BetheBlochSolid(bg);
...
NaMixture::NaMixture(const char* name, const char* title, 
  ...
  : Material(name,title,a,z,dens,radl,absl,elbuff) 
{
  // Constructor
  fNMix = 0;
  fAMix = fZMix = fWMix = 0;
