  ...
  ,fTrack()
{
  ...
  if (LUXELog::GetGlobalDebugLevel()>=2) {
    LUXEDebug(2,Form("XYZ: %+e %+e %+e PXYZ: %+e %+e %+e Q=%d",xyz[0],xyz[1],xyz[2],pxyz[0],pxyz[1],pxyz[2],sign));
    Print("etp");
  }
}
...
Bool_t TrkProbe::PropagateToZBxByBz(double z, double maxDZ, Double_t xOverX0, Double_t xTimesRho, Bool_t modeMC)
{
  ...
  // LUXEDebug(2,Form("from Z=%f to Z=%f, X/X0: %f X*rho:%f, max step:%f Mode:%d (%d steps)", GetZ(),z,xOverX0,xTimesRho,maxDZ,modeMC,nz));
...
Bool_t TrkProbe::ApplyMSEL(double x2X0, double xTimesRho)
{
  ...
  if (x2X0<0 || xTimesRho>0) LUXEFatal(Form("Negative X/X0=%f or X*rho=%f",x2X0,xTimesRho));
  ...
  if (xTimesRho<-1e-7) {
    ...
    Double_t bg = p/fMass;
    Double_t dEmean=TrackParameter::BetheBlochSolid(bg)*xTimesRho; // mean loss
