...
//                                                                           //
// The LUXELog class is a singleton class. It allows to steer the output      //
...
// in the header file: LUXEDebug, LUXEInfo, LUXEWarning, LUXEError, LUXEFatal.    //
//                                                                           //
...
LUXELog* LUXELog::fgInstance = NULL;

Bool_t LUXELog::fgDebugEnabled = kTRUE;
Bool_t LUXELog::fgCoreEnabled = kFALSE;


/**
 * get root logger singleton instance
 */
LUXELog *LUXELog::GetRootLogger()
{
	if (fgInstance == NULL)
	{
		// creating singleton
		fgInstance =  new LUXELog;
	}
...
LUXELog::LUXELog(const LUXELog& log) :
  TObject(log),
...
// copy constructor
  ...
  Fatal("LUXELog", "copy constructor not implemented");
}

// NOT IMPLEMENTED!?
//_____________________________________________________________________________
LUXELog& LUXELog::operator = (const LUXELog& /*log*/)
{
// assignment operator
 ...
 * LUXERoot.LUXELog.EnableDebug
 * LUXERoot.LUXELog.GlobalLogLevel
 */
//_____________________________________________________________________________
void LUXELog::ReadEnvSettings()
{
...
// and from environment variables
  ...
  else if (gEnv->Defined("LUXERoot.LUXELog.EnableDebug"))
  {
    fgDebugEnabled = gEnv->GetValue("LUXERoot.LUXELog.EnableDebug", fgDebugEnabled);
    LUXEInfo(Form("debug %sabled", ((fgDebugEnabled) ? "en" : "dis")));
  }

  // global log level
  if (gEnv->Defined("LUXERoot.LUXELog.GlobalLogLevel"))
  {
    const char* type = gEnv->GetValue("LUXERoot.LUXELog.GlobalLogLevel", "");
    ...
    LUXEDebug(3, Form("global log level set to %d", fGlobalLogLevel));
  }

  // global debug level
  if (gEnv->Defined("LUXERoot.LUXELog.GlobalDebugLevel"))
  {
    Int_t level = gEnv->GetValue("LUXERoot.LUXELog.GlobalDebugLevel", Int_t(fGlobalLogLevel - kDebugOffset));
    if (level < -kDebugOffset) level = kDebugOffset;
    fGlobalLogLevel = kDebugOffset + level;
    LUXEDebug(3, Form("global debug level set to %d", fGlobalLogLevel - kDebugOffset));
  }

  // module debug level
  if (gEnv->Defined("LUXERoot.LUXELog.ModuleDebugLevel"))
  {
    TString levels = gEnv->GetValue("LUXERoot.LUXELog.ModuleDebugLevel", "");
    ...
    while (const char* module = strtok(p, " "))
    {
      ...
      *(pos++) = '\0';
      ...
      LUXEDebug(3, Form("debug level for module %s set to %d", module, level));
    }
  }

  // class debug level
  if (gEnv->Defined("LUXERoot.LUXELog.ClassDebugLevel"))
  {
    TString levels = gEnv->GetValue("LUXERoot.LUXELog.ClassDebugLevel", "");
    char* p = const_cast<char*>(levels.Data());
    ...
    while (const char* className = strtok(p, " "))
    {
      ...
      *(pos++) = '\0';
      ...
      LUXEDebug(3, Form("debug level for class %s set to %d", className, level));
    }
  }

  // general output stream
  if (gEnv->Defined("LUXERoot.LUXELog.Output"))
  {
    TString stream = gEnv->GetValue("LUXERoot.LUXELog.Output", "Standard");

    if (stream.CompareTo("standard", TString::kIgnoreCase) == 0)
    {
      SetStandardOutput();
      LUXEDebug(3, "output stream set to standard output for all types");
    }
    else if (stream.CompareTo("error", TString::kIgnoreCase) == 0)
    {
      SetErrorOutput();
      LUXEDebug(3, "output stream set to error output for all types");
    }
    else if (!stream.IsNull())
    {
      SetFileOutput(stream);
      LUXEDebug(3, Form("output stream set to file %s for all types", stream.Data()));
    }
  }
  ...
  for (Int_t iType = kFatal; iType < kMaxType; iType++)
  {
    TString name("LUXERoot.LUXELog.Output.");
    ...
    if (gEnv->Defined(name))
    {
      ...
      if (stream.CompareTo("standard", TString::kIgnoreCase) == 0)
      {
        ...
        LUXEDebug(3, Form("output stream set to standard output for type %s", typeNames[iType]));
      }
      else if (stream.CompareTo("error", TString::kIgnoreCase) == 0)
      {
        SetErrorOutput(EType_t(iType));
        LUXEDebug(3, Form("output stream set to error output for type %s", typeNames[iType]));
      }
      else if (!stream.IsNull())
      {
        SetFileOutput(EType_t(iType), stream);
        LUXEDebug(3, Form("output stream set to file %s for type %s", stream.Data(), typeNames[iType]));
      }
    }
  }

  // handling of root error messages
  if (gEnv->Defined("LUXERoot.LUXELog.HandleRootMessages"))
  {
    Bool_t on = gEnv->GetValue("LUXERoot.LUXELog.HandleRootMessages", kTRUE);
    SetHandleRootMessages(on);
    LUXEDebug(3, Form("handling of root messages %sabled", ((on) ? "en" : "dis")));
  }
  ...
  {
    TString name("LUXERoot.LUXELog.Print");
    ...
    if (gEnv->Defined(name))
    {
      ...
      LUXEDebug(3, Form("printing of %s %sabled for all types", settingNames[iSetting], ((on) ? "en" : "dis")));
    }
    ...
    for (Int_t iType = kFatal; iType < kMaxType; iType++)
    {
      ...
      if (gEnv->Defined(nameType))
      {
        ...
        settings[iSetting][iType] = on;
        LUXEDebug(3, Form("printing of %s %sabled for type %s", settingNames[iSetting], ((on) ? "en" : "dis"), typeNames[iType]));
      }
    }
  }

  // repetition of messages
  if (gEnv->Defined("LUXERoot.LUXELog.PrintRepetitions"))
  {
    Bool_t on = gEnv->GetValue("LUXERoot.LUXELog.PrintRepetitions", kTRUE);
    fPrintRepetitions = on;
    LUXEDebug(3, Form("printing of message repetitions %sabled", ((on) ? "en" : "dis")));
  }
...
//_____________________________________________________________________________
void LUXELog::RootErrorHandler(Int_t level, Bool_t abort, 
...
// new error handler for messages from root
  ...
  LUXELog::Message(level, message, "ROOT", NULL, location, NULL, 0);
}


// DEPRECATED: USE A CONFIGURATION FILE INSTEAD
//_____________________________________________________________________________
void LUXELog::EnableDebug(Bool_t enabled)
{
...
void LUXELog::EnableCoreDump(Bool_t enabled)
{
// enable or disable debug output
...
//_____________________________________________________________________________
void LUXELog::SetGlobalLogLevel(EType_t type)
{
// set the global debug level

  // TO BE DELETED
  if (!fgInstance) new LUXELog; 
  fgInstance->fGlobalLogLevel = type;
}

//_____________________________________________________________________________
Int_t LUXELog::GetGlobalLogLevel()
{
...
//_____________________________________________________________________________
void LUXELog::SetGlobalDebugLevel(Int_t level)
{
// set the global debug level
...
//_____________________________________________________________________________
Int_t LUXELog::GetGlobalDebugLevel()
{
// get the global debug level
...
//_____________________________________________________________________________
void LUXELog::SetModuleDebugLevel(const char* module, Int_t level)
{
...
//_____________________________________________________________________________
void LUXELog::ClearModuleDebugLevel(const char* module)
{
...
//_____________________________________________________________________________
void LUXELog::SetClassDebugLevel(const char* className, Int_t level)
{
...
//_____________________________________________________________________________
void LUXELog::ClearClassDebugLevel(const char* className)
{
...
//_____________________________________________________________________________
void LUXELog::SetStandardOutput()
{
...
//_____________________________________________________________________________
void LUXELog::SetStandardOutput(EType_t type)
{
...
//_____________________________________________________________________________
void LUXELog::SetErrorOutput(EType_t type)
{
...
//_____________________________________________________________________________
void LUXELog::SetFileOutput(const char* fileName)
{
...
//_____________________________________________________________________________
void LUXELog::SetFileOutput(EType_t type, const char* fileName)
{
...
//_____________________________________________________________________________
void LUXELog::CloseFile(Int_t type)
{
...
//_____________________________________________________________________________
FILE* LUXELog::GetOutputStream(Int_t type)
{
...
//_____________________________________________________________________________
void LUXELog::SetHandleRootMessages(Bool_t on)
{
...
//_____________________________________________________________________________
void LUXELog::SetPrintType(Bool_t on)
{
...
//_____________________________________________________________________________
void LUXELog::SetPrintType(EType_t type, Bool_t on)
{
...
//_____________________________________________________________________________
void LUXELog::SetPrintModule(Bool_t on)
{
...
//_____________________________________________________________________________
void LUXELog::SetPrintModule(EType_t type, Bool_t on)
{
...
//_____________________________________________________________________________
void LUXELog::SetPrintScope(Bool_t on)
{
...
//_____________________________________________________________________________
void LUXELog::SetPrintScope(EType_t type, Bool_t on)
{
...
//_____________________________________________________________________________
void LUXELog::SetPrintLocation(Bool_t on)
{
...
//_____________________________________________________________________________
void LUXELog::SetPrintLocation(EType_t type, Bool_t on)
{
// switch on or off the printing of the file name and line number 
// for the given message type
...
//_____________________________________________________________________________
void LUXELog::SetPrintRepetitions(Bool_t on)
{
...
//_____________________________________________________________________________
void LUXELog::WriteToFile(const char* name, Int_t option)
{
...
//_____________________________________________________________________________
UInt_t LUXELog::GetLogLevel(const char* module, const char* className) const
{
...
//_____________________________________________________________________________
Int_t LUXELog::GetDebugLevel(const char* module, const char* className)
{
...
//_____________________________________________________________________________
void LUXELog::PrintMessage(UInt_t type, const char* message, 
...
//_____________________________________________________________________________
void LUXELog::Message(UInt_t level, const char* message, 
		     const char* module, const char* className,
		     const char* function, const char* file, Int_t line)
{
// print a log message
  ...
  if (type == kFatal) {
    fgInstance->PrintMessage(type, "aborting execution due to LUXEFatal", 
                             module, className, function, file, line);
    delete fgInstance;
    if (gSystem) {
      gSystem->StackTrace();
      if (fgCoreEnabled) MakeCoreDump("core.LUXERoot");
      gSystem->Abort();
    } else {
      if (fgCoreEnabled) MakeCoreDump("core.LUXERoot");
      ::abort();
...
//_____________________________________________________________________________
void LUXELog::Debug(UInt_t level, const char* message, 
		   const char* module, const char* className,
		   const char* function, const char* file, Int_t line)
{
...
//_____________________________________________________________________________
Int_t LUXELog::RedirectStdoutTo(EType_t type, UInt_t level, const char* module, 
...
//_____________________________________________________________________________
Int_t LUXELog::RedirectStderrTo(EType_t type, UInt_t level, const char* module, 
                               const char* className, const char* function,
                               const char* file, Int_t line, Bool_t print)
{
...
//_____________________________________________________________________________
Int_t LUXELog::RedirectTo(FILE* stream, EType_t type, UInt_t level, 
                         const char* module, const char* className,
                         const char* function, const char* file, Int_t line,
			 Bool_t print)
{
// redirect the standard (error) output stream to the stream of the given type
  ...
  if (type + level > GetLogLevel(module, className)) { // /dev/null
    if(!freopen("/dev/null", "a", stream)) LUXEWarning("Cannot reopen /dev/null");
    ...
    if(!freopen(fFileNames[type], "a", stream)) LUXEWarning(Form("Cannot reopen %s",fFileNames[type].Data()));
...
//_____________________________________________________________________________
void LUXELog::RestoreStdout(Int_t original)
{
...
//_____________________________________________________________________________
void LUXELog::RestoreStderr(Int_t original)
{
...
//_____________________________________________________________________________
ostream& LUXELog::Stream(EType_t type, UInt_t level,
                        const char* module, const char* className,
                        const char* function, const char* file, Int_t line)
{
...
//_____________________________________________________________________________
ostream& LUXELog::GetStream(EType_t type, UInt_t level,
                           const char* module, const char* className,
                           const char* function, const char* file, Int_t line)
{
// get the stream object for the given output type
...
void  LUXELog::SetStreamOutput(ostream* stream)
{
  ...
  // LUXELog class just writes to it

  for (Int_t iType = kFatal; iType < kMaxType; iType++) {
    SetStreamOutput((LUXELog::EType_t)iType, stream);
  }
}

void  LUXELog::SetStreamOutput(EType_t type, ostream* stream)
{
...
void  LUXELog::SetLogNotification(LUXELogNotification pCallBack)
{
  // set a notification callback function for log messages of all types

  for (Int_t iType = kFatal; iType < kMaxType; iType++) {
    SetLogNotification((LUXELog::EType_t)iType, pCallBack);
  }
}

void  LUXELog::SetLogNotification(EType_t type, LUXELogNotification pCallBack)
{
...
void  LUXELog::PrintString(Int_t type, FILE* stream, const char* format, ...)
{
...
void LUXELog::MakeCoreDump(const char *fout){
  //
  ...
  //
  // Example use - make default core file for current process:  LUXELog::MakeCoreDump(0)
  //
  //
  // Automatic core dump creation in case of the LUXEFatal can be specified using
  // static void  EnableCoreDump(Bool_t enabled);
  // Core dump is created in addition to the stack trace ()  
  // marian.ivanov@cern.ch
  //
  if (!gSystem) return;
  printf("LUXELog::MakeCoreDump\n");
  if (fout){
...
void LUXELog::TestException(Int_t level){
  //
  // Dummy function to throw exception
  //
  printf("LUXELog::TestException(%d)\n",level);
  if (level>0){
    level--;
    TestException(level);
  }else{
