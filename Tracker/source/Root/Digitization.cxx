...
TPolyLine3D* TrackLine3d(const TrkProbe* source, Double_t zMax, Double_t step=1, Color_t col=kBlack)
{	 
	 double xyz[3];
	 source->GetXYZ(xyz);
...
TPolyMarker3D* TrackMarker3d(const TrkProbe* source, double zmin, double zmax, double zstep, Color_t col, bool doPrint=false, bool fullrange=false)
{
    TrkProbe tmp(*source);
...
TPolyLine3D* GetTrkLayer(TString side, double z, Color_t col)
{
   Int_t n=5;
...
void WriteGeometry(vector<TPolyMarker3D*>& polm, vector<TPolyLine3D*>& poll, TString process, vector<int>& inacc, vector<TPolyMarker3D*>& clusters, TString suff="")
{
   ...
   TPolyLine3D* stave1L = GetTrkLayer("L",300,kGreen+3);
   TPolyLine3D* stave1R = GetTrkLayer("R",300,kGreen+3);
   TPolyLine3D* stave2L = GetTrkLayer("L",310,kGreen+3);
   TPolyLine3D* stave2R = GetTrkLayer("R",310,kGreen+3);
   TPolyLine3D* stave3L = GetTrkLayer("L",320,kGreen+3);
   TPolyLine3D* stave3R = GetTrkLayer("R",320,kGreen+3);
   TPolyLine3D* stave4L = GetTrkLayer("L",330,kGreen+3);
   TPolyLine3D* stave4R = GetTrkLayer("R",330,kGreen+3);
   ...
   gSystem->Exec("mkdir -p "+storage+"/output/root");
   gSystem->Exec("mkdir -p "+storage+"/output/pdf");
   ...
   for(int iev=(fullloop)?0:evnt ; (fullloop)?iev<nev:iev==evnt ; iev++)
   {
      ...
      for(int igen=0 ; igen<ngenall ; igen++)
      {
         if(abs(pdgId->at(igen))!=11)
         {
			...
			//std::cout << "inside ngenall loop" << std::endl;
         //// all the rest
         ngen++;
			vector<int> vtmp;
         ...
         //std::cout << "after ptmp.SetXYZM" << std::endl;
         // prepare the probe
         ...
         nslv++;
			//std::cout << "after preparing the probe" << std::endl;
         // get the truth trajectory in the B-field
         TrkProbe* trutrk = det->GetTrkProbe();
			// double pxyztmp[3];
			// trutrk->GetPXYZ(pxyztmp);
			//std::cout << "after GetTrkProbe" << std::endl;
			int slvidx = nslv-1;
         wgt.push_back( wgt0->at(igen) );
         //std::cout << "after wgt push" << std::endl;
         xvtx.push_back( vx->at(igen) );
         //std::cout << "after xvtx push" << std::endl;
         yvtx.push_back( vy->at(igen) );
         //std::cout << "after yvtx push" << std::endl;
         zvtx.push_back( vz->at(igen) );
         //std::cout << "after zvtx push" << std::endl;
         crg.push_back( q );
         //std::cout << "after crg push" << std::endl;
         trkp4.push_back( ptmp );
         //std::cout << "after trkp4 push" << std::endl;
         ...
         //std::cout << "after trkpts push" << std::endl;
			trklin.push_back(TrackLine3d(trutrk,360,1,trkcol(ptmp.E())) );
         //std::cout << "after trklin push" << std::endl;
			...
			acc.push_back( 0 );
			//std::cout << "pushing to clusters vector" << std::endl;
			// get the reconstructed propagated to the vertex
         Cluster* cluster1 = det->GetTrkLayer(1)->GetMCCluster();
         Cluster* cluster2 = det->GetTrkLayer(3)->GetMCCluster();
         Cluster* cluster3 = det->GetTrkLayer(5)->GetMCCluster();
         Cluster* cluster4 = det->GetTrkLayer(7)->GetMCCluster();
			...
			clusters_type[slvidx].push_back( (process.Contains("bkg")) ? 0 : 1 );
			clusters_type[slvidx].push_back( (process.Contains("bkg")) ? 0 : 1 );
			//std::cout << "after reconstructed propagated to vertex" << std::endl;
			/// check acceptance
